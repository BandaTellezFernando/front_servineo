  ...
  useEffect(() => {
    const updatePosition = () => {
      ...
      if (targetElement && stepRef.current) {
        ...
        let left = rect.left + (rect.width / 2) - (stepRect.width / 2);

        // Ajustar posición según la preferencia
        if (step.position === 'top') {
          top = rect.top - stepRect.height - 20;
        }

        // Asegurar que no se salga de los bordes de la pantalla
        if (top + stepRect.height > viewportHeight - 20) {
          top = viewportHeight - stepRect.height - 20;
        }
        if (top < 20) {
          top = 20;
        }
        if (left + stepRect.width > viewportWidth - 20) {
          left = viewportWidth - stepRect.width - 20;
        }
        if (left < 20) {
          left = 20;
        }

        // Si el tooltip cubre el elemento, ajustar posición
        const tooltipOverlapsElement = 
          top < rect.bottom && 
          top + stepRect.height > rect.top &&
          left < rect.right && 
          left + stepRect.width > rect.left;

        if (tooltipOverlapsElement) {
